...
<%
	...
	String nbJourString = request.getParameter("nbJours");
	Integer nbJours = 2;
	if (nbJourString != null) {
		nbJours = new Integer(nbJourString);
	}
		
	String dateCalendrier = request.getParameter( "orderdate" ); 
	String dateDemandee = dateCalendrier;


	//Récupération des infos de consomation
	if (dateDonneesSession == null) {
		//On a encore rien récupéré on doit tout charger
		donneesPuissanceMap = ConnectionManager.getInstance().getDonneesPuissanceRefresh(nbJours,
				donneesPuissanceMap, dateDemandee);
...
<body>
	...
	<script type="text/javascript" src="js/googlejaspi.js"></script>
	<script type="text/javascript" src="js/calendarDateInput.js"></script>

	<table width="100%" border=0>
		<tr>
			<td width="40%" align="center"><img
				src="http://www.prevision-meteo.ch/uploads/widget/douvrin_0.png"
				width="550" height="200" /></td>
			<td width="20%" align="center"><h1><%=dateTitre%></h1></td>
			<td width="30%" align="right">

				<form method="post" action="index.jsp">
					Option d'affichage: <select id="optionConso" , name="optionConso"
						onChange="submit()">
						<option value="kwh" <%=selectedkWh%>>kWh</option>
						<option value="euros" <%=selectedEuros%>>Euros</option>
					</select> <br /> Nombre de jours: <select id="nbJours" , name="nbJours"
						onChange="submit()">
					...
					<br /> Raffraichir :<input type="checkbox" name="raffraichir"
						value="raffraichir" onchange="submit()" <%=checkedRaffraichir%>>
					<input id='pasRaffraichir' type='hidden' value='No'
						name='pasRaffraichir'>
				</form>
			</td>
			<td width="10%"></td>
